...
congested_links.to_csv('D:/BILAL/5. shortest path/daejon network master data/congested_links.csv', index = False)
#plotting the network data and check the speed variation across various links


# f_ntest.LINK_ID.value_counts()
# #counting unbiques
# items = Counter(f_ntest2.ROAD_ROUTE_ID).keys()
# len(items)

#converting our data into graph data
xx = sample.drop_duplicates(subset=['LINK_ID'])
# xx = xx.sample(n = 1000)
edgelist = xx[['START_NODE_ID','END_NODE_ID','LINK_LEN']]
...
ox.distance.get_nearest_nodes(g, X=xx['O_x'], Y=xx['O_y'], method='balltree')
